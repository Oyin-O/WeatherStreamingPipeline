...
# ── Step 3: Start Producer in new terminal tab ─────────────────
...
# ── Step 5: Start Spark Consumer in new terminal tab ──────────
echo -e "${YELLOW}▶ Starting Spark Consumer...${NC}"
osascript -e "
tell application \"Terminal\"
    do script \"cd '$PROJECT_DIR' && source '$VENV' && python -m spark.sparkconsumer\"
end tell"
...
# ── Step 7: Start Streamlit Dashboard in new terminal tab ─────
echo -e "${YELLOW}▶ Starting Streamlit Dashboard...${NC}"
osascript -e "
tell application \"Terminal\"
    do script \"cd '$PROJECT_DIR' && source '$VENV' && streamlit run dashboard/Streamlitdashboard.py\"
end tell"
